...
- 当前编码主线已开始，阶段 1、阶段 2、阶段 3 已完成，下一步进入阶段 4 的重复规则与实例展开最小闭环。
...
- 已完成 `upcoming_query` 的重复实例展开首版，当前已支持按天、周、月、年重复，并支持间隔值、循环截止日期与稳定 `occurrence_key`。
- 已补重复规则展开的首批 Rust 测试，覆盖按天展开、月底钳制与覆盖记录合并。
...
1. 先在 Rust 侧实现最小重复规则展开能力，并将范围限定在 `upcoming_query`
2. 优先完成按天/按周/按月/按年的窗口内实例展开与稳定 `occurrence_key`
3. 通过 Rust 单元测试锁定重复规则的时间边界、截止边界与排序行为
4. 阶段 4 后续如需前端接入，再单独补充实施计划并进入页面层
